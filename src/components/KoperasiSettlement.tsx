...
export const KoperasiSettlement: React.FC<KoperasiSettlementProps> = ({
  ...
  // Anti-double check on selected date
  const settlementsOnSelectedDate = settlements.filter(
    (s) => s.date === settlementDate
  );
            ...
            <div className="p-4 rounded-2xl bg-emerald-50/50 border border-emerald-200/80 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  />
                  {settlementsOnSelectedDate.length > 0 && (
                    <div className="mt-1.5 p-2 rounded-lg bg-amber-50 border border-amber-200 text-[11px] text-amber-900 font-semibold flex items-center gap-1.5">
                      <span className="text-amber-600">⚠️</span>
                      <span>Sudah ada {settlementsOnSelectedDate.length} setoran pada tanggal {formatDateIndo(settlementDate)} (Total: {formatRupiah(settlementsOnSelectedDate.reduce((acc, s) => acc + s.amountCollected, 0))}). Pastikan tidak dobel input.</span>
                    </div>
                  )}
                </div>
